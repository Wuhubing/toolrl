...
Assumed stack: Python, PostgreSQL, Docker, a standard RLHF library (e.g., TRL) as the base with custom GRPO implementation. Adjust if different.
...
Requirements:
- **SFT stage:** fine-tune the base 3B model on successful rollout trajectories (or human/expert-authored trajectories if available) to establish a reasonable initial policy
- **DPO stage:** construct preference pairs from rollouts (e.g., successful vs failed trajectories, or higher-reward vs lower-reward trajectories for the same task) and run DPO on top of the SFT checkpoint
- **GRPO stage:** group-relative policy optimization using the task-success reward from 2.3, on top of the DPO checkpoint
...
2. Sandbox provisioning/teardown + task environment `step()` interface
3. Execution tracer + deterministic replay (build this early, it makes debugging every later stage easier)
4. SFT training script + eval harness against held-out test set
5. DPO stage (preference pair construction + training)
6. GRPO stage (reward wiring from task success + training)
